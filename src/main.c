...
int main(void) {
  ...
  dielectric_t material_left = dielectric_create(1.5);
  ...
  sphere_t inside_sphere = sphere_create(vec3_create(-1, 0, -1), -0.4, &material_left.base); // Negative radius (inside sphere
  ...
  hittable_list_add(&world, &inside_sphere.base);
